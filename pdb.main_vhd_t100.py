...
if __name__ == '__main__':
    ...
    ##--- van hove self ---
    dist_col, vanhove_d_col, vanhove_d_o_4pir2_col = d1.vanhove_d_AN_o_4pir2_avg(1, 25.0, 0.1) # t*=1200 ps, so interval*=12
    print('van hove dist calculated')
    ##--- timer stop ---
    stop = timer.perf_counter()
    
    ##--- save vanhove func
    np.savetxt(    fn_prefix+'vhd_t100.dat', np.transpose( np.vstack( (dist_col, vanhove_d_col) )), fmt=['%f', '%f'], header='dist, vanhove_d'      )
    np.savetxt(    fn_prefix+'vhd_o_4pir2_t100.dat', np.transpose( np.vstack( (dist_col, vanhove_d_o_4pir2_col ) )), fmt=['%f', '%f'], header='dist, vanhove_d/4pi*r^2'      )
