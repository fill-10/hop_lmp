...
if __name__ == '__main__':
    ...
    #fn_prefix = '../Br_anti_'
    fn_prefix = '../Cl_anti_Xbond'
    #fn_prefix = '../Br_anti_Xbond'
    ##--- timer start ---
    ...
    for frame in d1.allframes:
        ...
        frame.L_CT = frame.L_atom[frame.L_atom['type']=='CLH'].reset_index(drop=True)
        #del frame.L_atom
    d1.wrapall_L()
    ##--- calcuate histograms ---
    # find the coordinated LP1s for each BRH
    norm_hist_atom, norm_hist_mol = d1.find_asso_AN_CT(4.0)
    ...
    for frame in d1.allframes:
        ...
        for (idx, row) in frame.L_AN.iterrows():
            ...
            for i in range(0, len(row['asso_atom']) ):
                ...
                sg_self = frame.L_atom[ frame.L_atom['id'] == (row['id']-1) ].iloc[0] #self 'CZ' as pd.Series
